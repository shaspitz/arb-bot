...
const erc20Contract = await ethers.getContractAt(erc20Abi, ERC20_ADDRESS);
 ...
 * @param  {} account
 */
async function manipulateTestNetPrice(token, account) {
    console.log(`\nBeginning Swap...\n`);
    ...
    const receipt = await routerToUse.swapExactTokensForTokens(
        amountInSmallestDecimal, 0, path, account, deadline, options);
